...
Task: generate compact item cards for RSS inbox items. Do not decide deletion, folding, notification, or cluster membership.
...
- Keep the card short and reusable for relation judging; avoid long prose.
...
- Preserve product/object distinctions that affect event matching, such as Finance vs Health Sources, Recommended Connectors vs Projects memory, or PHP SDK vs general Firecrawl.
...
- For social posts, strip xgo/tweet boilerplate from the semantic summary when possible, but keep concrete URLs/product names that identify the event.
